...
以下の２つに答えるとflagがもらえる．
1. 楕円曲線上のある点の$y$座標が与えられるので対応する$x$座標を答える．
2. $P=(x,y)$に対して，$-P=aP$となる$a\in \mathbb{Z}_{\geq 0}$を答える．

#### find $x$

楕円曲線$E(\mathbb{F_{p}}):y^{2}=x^{3}+7$において，ある$P\in E(\mathbb{F}_p)$の$y$座標（$y_{P}$とする）が与えられるので，対応する$x$座標（$x_{P}$とする）を答えれば第一関門突破です．
...
すなわち，$Q=-P$となるような$a$を見つければよい．
残念ながら，$a\geq 0$であるから$a=-1$とはできません．

そこで，$P$の位数$n$から１を引いた値を$a$にすれば$-P=aP$を満たせます．
